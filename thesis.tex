...
provably enumerate the set of totally split $J_0(N)$, show that computing the
rational torsion subgroup is algorithmic, and later give techniques for
...
simple abelian subvariety $A$ of $J_0(N)$ under mild conditions. In particular,
when the Hecke algebra of $A$ is isomorphic to a maximal order of a number
field, we are able to enumerate the isogeny class. This will allow us to build
a table of isogeny graphs of simple abelian subvarieties of $J_0(N)$.
}
\copyrightpage
...
The following algorithm works for these Ns. %TODO
\begin{algorithm}
