...
\label{sub:special_maps}


\subsection{Optimal quotients and subvarieties}%
\label{sub:optimal}
...
\subsection{Connectedness of Hecke Algebra}%
\label{sub:connectedness_of_hecke_algebra}
...
\subsection{Simple abelian subvarieties as image of degeneracies}
\begin{algorithm}{Simple abelian subvarieties as image of degeneracies}%
    \label{alg:simple_degen}
    Given a simple abelian subvariety $A\substeq J_0(N)$. This algorithm
...
\subsection{Decomposing and Verifying Abelian subvarieties}
...
\begin{algorithm}{Decomposing and Verifying Abelian Subvarieties}
    ...
    'NO'. Let $\T=\{T_1,\ldots\}$ be the 0ecke algebra of $J$.
    \begin{enumerate}
        \item{} [Decompose into potential 0ecke eigenspaces]
...
\subsection{Homomorphism spaces}

\subsection{Isogeny and isomorphism testing}
...
\subsection{Strategies for computing the rational torsion subgroup}
...
Let $I$
and $s$ be
the star-involution on $J$. Then $J(\QQ)_\tor\subseteq J(\RR)=J[s]$.
Stein showed that $J(\QQ)_\tor\subseteq J[\eta_\ell]$. So 
\[
    J(\QQ)_\tor \subseteq J[I^*],
\]

This next lemma is a useful observation for computations but not interesting
from a theory points of view. The $E$ here will often be Stein's Real
Eisenstein Kernel.
\begin{lemma}
