...
\begin{theorem}
    There are 45 totally split rank-0 Jacobians $J_0(N)$. The Generalized Ogg
    Conjecture has been verified for all but 9 $N$'s. In these cases, they are
    bounded by a power of 2. In particular, let $b(N)=\log_2
    ([J_0(N)(\QQ)_\tor:C(N)(\QQ)])$, where $C(N)$ is the cuspidal subgroup of
    $N$. The 9 $N$'s where we have not verified the Generalize Ogg Conjecture
    is given by
\end{theorem}
\begin{algorithm}
